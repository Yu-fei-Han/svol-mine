...
def find_closest_number(numbers, x):
    differences = np.abs(np.array(numbers) - x)
...
def get_camera_params(uv, pose, intrinsics):
    ...
    # u, v = get_uv(world_coords, pose, intrinsics)
...
def get_uv(world_coords, pose, intrinsics):
    """
    uv: (B, N, 2)
    pose, intrinsics: (B, 4, 4)
    """
    ...
    # [(B, N, 3) - (B, 1, 3)] @ (1, 3, 3) -> (B, N, 3)
    pts_cam = (world_coords - p[:, :3, 3].view(-1, 1, 3)) @ p[:, :3, :3]
    x_lift = pts_cam[:, :, 0]
    y_lift = pts_cam[:, :, 1]
    z = pts_cam[:, :, 2] # (B, N)

    y = y_lift / z * fy + cy
    x = x_lift / z * fx + cx - cy*sk/fy + sk*y/fy
