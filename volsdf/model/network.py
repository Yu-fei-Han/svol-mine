...
from volsdf.model.embedder import *
from volsdf.model.density import LaplaceDensity
...
class ImplicitNetwork(nn.Module):
    ...
    def gradient(self, x, with_sdf=False):
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input, fast=-1):
        ...
        z_vals, z_samples_eik = self.ray_sampler.get_z_vals(ray_dirs, cam_loc, self, fast=fast, iter_step=iter_step) # (N_rays, N_samples)
        N_samples = z_vals.shape[1] # = 1 + 64 + 32 + 1

        points = cam_loc.unsqueeze(1) + z_vals.unsqueeze(2) * ray_dirs.unsqueeze(1) # (N_rays, N_samples, 3)
        ...
        weights, dists = self.volume_rendering(z_vals, sdf) # (N_rays, N_samples)
