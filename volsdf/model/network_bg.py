...
from volsdf.model.network import ImplicitNetwork, RenderingNetwork
from volsdf.model.density import LaplaceDensity, AbsDensity
from volsdf.model.ray_sampler import ErrorBoundSampler
...
class VolSDFNetworkBG(nn.Module):
    ...
    def forward(self, input, fast=-1):
        ...
        z_vals, z_samples_eik = self.ray_sampler.get_z_vals(ray_dirs, cam_loc, self, fast=fast)

        z_vals, z_vals_bg = z_vals #  (z_vals, z_vals_inverse_sphere)
        z_max = z_vals[:,-1]
        z_vals = z_vals[:,:-1]
        ...
        if not self.training:
            # use nearest viewing direction in training set, slightly better performance
            ray_dirs_near, _ = rend_util.get_camera_params(uv, input["near_pose"].cuda(), intrinsics)
            ray_dirs_near = ray_dirs_near.reshape(-1, 3)
            dirs_flat = ray_dirs_near.unsqueeze(1).repeat(1,N_samples,1).reshape(-1, 3)
        rgb_flat = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors)
        rgb = rgb_flat.reshape(-1, N_samples, 3)

        weights, bg_transmittance, dists = self.volume_rendering(z_vals, z_max, sdf)
        ...
        bg_points, bg_depth_vals = self.depth2pts_outside(bg_locs, bg_dirs, z_vals_bg)  # [..., N_samples, 4]
        ...
        if not self.training:
            bg_dirs_flat = ray_dirs_near.unsqueeze(1).repeat(1,N_bg_samples,1).reshape(-1, 3)
        ...
        weights_all = torch.cat([weights, bg_transmittance[:, None]*bg_weights], 1)
        depth_vals_all = depth_scale * torch.cat([z_vals, bg_depth_vals], 1)
        depth_values_all = torch.sum(weights_all * depth_vals_all, 1, keepdims=True) / (weights_all.sum(dim=1, keepdims=True) +1e-8)

        depth_vals = z_vals * depth_scale
        depth_values = torch.sum(weights * depth_vals, 1, keepdims=True) / (weights.sum(dim=1, keepdims=True) +1e-8)
        ...
        output = {
            ...
            'depth_values_all': depth_values_all,
    ...
    def volume_rendering(self, z_vals, z_max, sdf):
        ...
        return weights, bg_transmittance, dists
    ...
    def depth2pts_outside(self, ray_o, ray_d, depth):
        ...
        d1 = - o_dot_d / torch.sum(ray_d * ray_d, dim=-1)
