...
import math
from helpers.utils import tocuda
...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, prob_volume_init=None, view_weights=None):
        ...
        # # entropy
        # prob_volume = torch.clamp(prob_volume, 1e-8, 1.-1e-8)
        # entropy = (-prob_volume*torch.log(prob_volume) - (1-prob_volume)*torch.log(1-prob_volume))/(-math.log(0.5))
        # entropy = torch.sum(entropy, axis=1) # (1, H, W)
        # entropy = 1. - entropy / entropy.max()

        with torch.no_grad():
            photometric_confidence = torch.max(prob_volume, dim=1)[0]
        if view_weights == None:
            view_weights = torch.cat(view_weight_list, dim=1) # [B, Nview, H, W]
            # "view_weight":view_weight[0]
...
class TransMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        logger.debug("**********netphs:{}, depth_intervals_ratio:{},  grad:{}, chs:{}************".format(ndepths, depth_interals_ratio, self.grad_method, self.cr_base_chs))
    ...
    def forward(self, stage_idx, sample_cuda,
            features, extra, outputs, int_r, depth=None, inverse_depth=False, prevent_oom=False):
        ...
        depth_range_samples = get_depth_range_samples(cur_depth=cur_depth,
                ...
                max_depth=depth_max,
                min_depth=depth_min,
                use_inverse_depth=inverse_depth)
